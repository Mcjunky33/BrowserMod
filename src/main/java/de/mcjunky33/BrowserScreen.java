...
public class BrowserScreen extends Screen {
    ...
    protected void init() {
        ...
        if (currentTab != null && currentTab.browser != null) {
            ...
            String screenFixJS =
                    "window.mcef_fs_func = function() {" +
                            "  var style = document.getElementById('mcef-fullscreen-style');" +
                            "  if (!style) {" +
                            "    style = document.createElement('style');" +
                            "    style.id = 'mcef-fullscreen-style';" +
                            "    style.innerHTML = ` " +
                            "      .forced-fs { position: fixed !important; top: 0 !important; left: 0 !important; width: 100vw !important; height: 100vh !important; z-index: 2147483647 !important; background: black !important; } " +
                            "      .forced-fs ~ * { display: none !important; } " +
                            "      body.has-forced-fs { overflow: hidden !important; } `;" +
                            "    document.head.appendChild(style);" +
                            "  }" +
                            "  Element.prototype.requestFullscreen = function() { " +
                            "    this.classList.add('forced-fs'); " +
                            "    document.body.classList.add('has-forced-fs'); " +
                            "    return Promise.resolve(); " +
                            "  };" +
                            "  document.exitFullscreen = function() { " +
                            "    var e = document.querySelector('.forced-fs'); " +
                            "    if(e) e.classList.remove('forced-fs'); " +
                            "    document.body.classList.remove('has-forced-fs'); " +
                            "    return Promise.resolve(); " +
                            "  };" +
                            "}; window.mcef_fs_func();";
        ...
        if (!isFullscreen) {
            ...
            if (!isFullscreen) {
                addRenderableWidget(Button.builder(Component.literal("?"), b -> {
                            getTabManager().prepareAndLoadWelcomePage();
                            refreshGui();
                        })
                        .bounds(this.width - 25, this.height - 22, 20, 20)
                        .tooltip(Tooltip.create(Component.literal("Help & Credits")))
                        .build());
            ...
            addRenderableWidget(
                    Button.builder(Component.literal("🎬"), b -> {
                            ...
                            .bounds(xPos, yPos, 20, 20)
                            .tooltip(Tooltip.create(Component.literal("Stream Browser on blocks (only BETA can be crash or have performance problems) ")))
                            .build()
            );
    ...
    @Override
    public void extractRenderState(net.minecraft.client.gui.GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partial) {
        validateIndices();
        TabManager.BrowserTab currentTab = tabManager.getCurrent();
        if (currentTab == null || currentTab.browser == null) {
        ...
        if (isFullscreen) {
            ...
            try {
                this.extractBackground(guiGraphics, mouseX, mouseY, partial);
            } catch (Exception e) {
        ...
        if (currentBrowser.isTextureReady()) {
            Identifier tex = currentBrowser.getTextureIdentifier();

            if (isFullscreen) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, tex, 0, 0, 0.0f, 0.0f, width, height, width, height);
            } else {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, tex, 20, UI_HEIGHT + 10, 0.0f, 0.0f, width - 40, height - UI_HEIGHT - 35, width - 40, height - UI_HEIGHT - 35);
            }
        }
        ...
        if (this.isFullscreen && this.fullscreenMsgTimer > 0) {
            ...
            guiGraphics.textWithBackdrop(this.font, fsHint, xPos, 20, textWidth, 0xFFFFFFFF);

            this.fullscreenMsgTimer--;
        }

        if (!isFullscreen) {
            renderLoadingBar(guiGraphics, currentBrowser);
            if (urlBar != null) {
                ...
                urlBar.extractRenderState(guiGraphics, mouseX, mouseY, partial);
            }
            super.extractRenderState(guiGraphics, mouseX, mouseY, partial);
        }
    }
    ...
    private void renderLoadingBar(GuiGraphicsExtractor guiGraphics, com.cinemamod.mcef.MCEFBrowser browser) {
        if (browser.isLoading() && urlBar != null) {
            int x = urlBar.getX();
            int y = urlBar.getY() - 2;
            int w = urlBar.getWidth();

            guiGraphics.fill(x, y, x + w, y + LOADING_BAR_HEIGHT, 0x55000000);
            ...
            if (segmentEnd > segmentStart) {
                guiGraphics.fill(segmentStart, y, segmentEnd, y + LOADING_BAR_HEIGHT, LOADING_BAR_COLOR);
            }
        }
    }
    ...
    public boolean charTyped(CharacterEvent event) {
        ...
        if (!tabManager.tabs.isEmpty()) {
            if ((pipActive && pipInteracting) || !isFullscreen) {

                tabManager.getCurrent().browser.sendKeyTyped((char) event.codepoint(), 0);
    ...
    private void registerAddressHandler() {
        if (addressHandler == null) {
            addressHandler = new org.cef.handler.CefDisplayHandlerAdapter() {
                @Override
                public void onAddressChange(org.cef.browser.CefBrowser b, org.cef.browser.CefFrame frame, String url) {
                    if (frame.isMain() && urlBar != null) {
                        minecraft.execute(() -> {
                            if (!urlBar.isFocused()) urlBar.setValue(url);
                        });
                    }
                }
            };
            com.cinemamod.mcef.MCEF.getClient().addDisplayHandler(addressHandler);
        }
    }
    ...
    public void onClose() {
        if (addressHandler != null) {
            com.cinemamod.mcef.MCEF.getClient().removeDisplayHandler(addressHandler);
            addressHandler = null;
        }
        tabManager.save();
        super.onClose();
